...
Для удобства локального запуска проект можно поднять одной командой через Docker Compose.
...
Из папки `my-app` выполните:

```bash
docker compose up --build
```

Не забудьте проверить, что у вас загружена и запущена ollama. Обратитесь к пункту выше. 
...
- **Docker Compose** – добавлен для упрощения локального запуска проекта: клиент и сервер разворачиваются в отдельных контейнерах и поднимаются одной командой.
